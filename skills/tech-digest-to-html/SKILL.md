...
**⑥ 渲染级视觉核验（headless Chrome + 整页切片裁读）**
DOM 垫片能证明脚本不崩、槽位不空，**证明不了「画出来好不好看」**。手写内联 SVG 的走线重叠、徽章挤成一团、某类文本被整段染成强调色——这几类缺陷静态自检 8 项全绿、冒烟测试 0 异常，却只有真实渲染才看得见。定稿前必须跑一次像素级目视：

```bash
CHROME="/Applications/Google Chrome.app/Contents/MacOS/Google Chrome"
"$CHROME" --headless=new --disable-gpu --hide-scrollbars \
  --virtual-time-budget=3000 --window-size=1500,11200 \
  --screenshot=/tmp/shot_full.png <目标.html>
```
- **整页长截图**用「超大 `--window-size` 高度」一次性拿全页（实测 1500×11200 可行），不要用注入 `window.scrollTo` 的方案——headless 下 load 后滚动往往不生效，会拿到三张一模一样的空白图（已踩）。
- **读图前先切片**：`/usr/bin/python3`（系统 Python 自带 PIL，托管 Python 无 PIL）把长图按高度切 4 片，再 `resize(0.72)` 落盘，逐片 `Read`。整张 11200px 直接读会被过度降采样，细节全丢。
- **主题双验**：另存一份把 `<html data-theme>` 与 JS 默认分支同时改掉的临时副本渲染一次，确认浅色态下表格、卡片、沙盘同样可读（只改属性不生效，见 §8.6）。
- **SVG 走线审计**：手绘多区域架构图时，先按 `viewBox` 坐标把每条边的贝塞尔中点算出来，检查两条边的走廊间距是否 ≥ 20px、徽章是否压在别的连线上。本例 `⑨` 曲线最初贴着 `⑦` 竖直走廊走（`x≈404` vs `x≈402`），肉眼一看即知需改道。
- **图表标注避让审计（2026-09-15 实证）**：曲线图上的文字标注（如「引爆点」「目标阈值」）默认放在阈值线一侧的固定偏移处，**在参数域的另一端必然贴线**。本例标注基线距曲线仅 **7.61px**，静态 8 项自检、运行时冒烟、逐槽位一致性**三项全绿**——它们都不看几何间距。正确做法有两步：① 让标注位置**自适应**，按标注 x 跨度（`lx-22, lx, lx+22`）采样曲线 y，取「上方 / 下方」中最小间距更大的一侧；② 交付前跑**全参数域扫描**，遍历所有滑块组合求「标注-曲线最小间距」的全局最小值，本例 21090 组组合的最小值由 7.61px 提升到 **15.02px**（判定阈值 ≥ 12px）。只验默认参数等于没验。
- **陷阱**：`qlmanage -t` 抓 HTML 缩略图在沙箱下会 `sandbox initialization failed`，不可用；macOS 无 `timeout` 命令（zsh 报 `command not found`），长命令用工具自身的 `timeout` 参数。

**⑥-补 · 长文档超出单张截图上限时的切片渲染法（2026-09-11 实证）**
`--window-size` 的高度受**进程内存**约束，不是可以随意加大的：1500×13000（约 78 MB 帧缓冲）可稳定产出，1500×17000（约 102 MB）**连续 5 次全部失败**（无产出 / 被 SIGTERM）。当文档实际高度超过上限（本例正文约 14500px），不要试图靠加大视口或负 margin 偏移（`#main{margin-top:-7000px}` 同样失败）硬扛，改用**切片渲染**：

```bash
# 1) 只抽取「尚未核验的尾段」+ 复用原文档的 <head>（含全部 CSS）
python3 - <<'PY'
import io
s = io.open(F, encoding='utf-8').read()
head = s[:s.index('<body>')]                      # 复用 <style>，保证视觉一致
tail = s[s.index('<h3 id="s5-3">'):s.index('</main>')]
io.open('/tmp/tail.html','w',encoding='utf-8').write(
    head + '<body><main id="main">' + tail + '</main>' + s[s.index('<script>'):])
PY
# 2) 渲染这个短页（本例尾段仅 6520px，一次成功）
"$CHROME" --headless=new --disable-gpu --hide-scrollbars \
  --user-data-dir=/tmp/pf --virtual-time-budget=4000 \
  --window-size=1500,9000 --screenshot=/tmp/tail.png /tmp/tail.html
```
- 切片的**视觉保真度与整页一致**（同一份 CSS、同一 `#main` 盒模型），但少了侧栏与顶部工具条——侧栏已在整页图里核验过，尾段无需重复。
- 读图前仍按高度切 4 片再 `resize(0.74)`，逐片 `Read`。
- **先用 `numpy` 找内容底边**再切：`np.where(np.asarray(im.convert('L')).std(axis=1) > 1.0)[0].max()`，避免把大片空白也切进图里浪费分辨率。

**⑥-补2 · zsh 通配符与残留进程（2026-09-11 实证）**
- `rm -rf po*` 在 zsh 下**无匹配时直接报 `no matches found: po*` 并中止整个复合命令**（后续步骤全不执行）。清理临时 profile 目录一律写成 `rm -rf ./po* 2>/dev/null || true`，或先 `setopt NULL_GLOB`。
...
| **第 7 项保留前缀误判（2026-09-11 实证）** | 把沙盘**区块容器**命名为 `sim-spec` / `sim-tier` / `sim-buy`（`sim` 是 simulation 的自然缩写） | `数据显示容器 #sim-spec 未在 JavaScript 中发现回写逻辑` | **保留前缀不止 `out-` 一个**。检测器把 `val-`、`sim-`、`res-`、`out-`、`stat-`、`usl-`、`pf-` **七个**前缀全部识别为回显槽位。任何**纯静态容器**（沙盘 `<section>`、说明区块、章节包裹层）的 ID 必须避开这七个前缀，改用 `box-` / `sec-` / `wrap-` 等中性前缀。判定方法：`grep -n 'startswith(prefix)' validate_html.py` 读源码而非猜 |
| **反查脚本重写常量映射导致假阳性（2026-09-15 实证）** | 独立写一个 Node 诊断脚本复算图表坐标时，把页面里的 `gyCost(p)`（`p=20`）手写成 `gyLog(1)`（对应成本 1 美元） | 诊断输出 `JS 重算标签 y = 131.79，静态初值 y = 118.42 → 不一致 FAIL`，看似页面有 bug，实际是**反查脚本自己算错了口径** | 反查 / 诊断脚本**必须复用页面同一份常量与映射函数**（从 HTML 里 `eval` 出 `GEO`、`gx`、`gyLog` 等，或至少逐字复制），**禁止凭记忆重写**。凡诊断脚本报「不一致」，先自查脚本口径，再怀疑页面——与 §Stage 5.5 ⑤「期望值禁止手工转录」是同一类错误的反向表现 |
| **同文件并行编辑静默丢失（2026-09-11 实证）** | 在同一条消息里对**同一文件**发起 3 个 Edit 调用 | 三个调用**全部**返回 `Successfully edited`，但磁盘上只有**第一个**生效，后两个被静默覆盖丢弃 | 同一文件的多处替换**必须串行**（分多轮）或在**一次**原子写中完成（Python 一次性读出→全部 replace→一次写回）。改完必须 `grep` 复核每一处目标串是否真的落地——**工具的成功回执不是证据** |
